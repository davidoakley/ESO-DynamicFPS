...
local function setActiveOnEvent(eventName, event)
  EVENT_MANAGER:RegisterForEvent(FPSManager.name, event, function()
...
local function onLogoutHook(_)
  ...
  logger:Info("FPSManager: onLogout; restoring to "..FPSManager.savedVars.fixedFPS.."fps")
...
function FPSManager.RegisterCallbacks()
  ...
  for eventCode, data in pairs(callbacks) do
    ...
    if data.callback ~= nil then
      EVENT_MANAGER:RegisterForEvent(FPSManager.name, eventCode, data.callback)
    else
